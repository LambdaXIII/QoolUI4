...
**分层原则（测什么放哪层）**：
- 组件的 C++ 函数 → C++ 测试（core/）；组件 QML 面的行为契约（stops 声明、信号、QML 属性）→ QML 测试（对应模块批次）
- 不在 C++ 层模拟 QML 引擎驱动 QML 组件（`QQmlListReference` 驱动 list、手动驱动 QQuickItem 场景）——那是用错误工具测错误面，崩溃/挂起/平台问题接踵而至
- 算法属于被调用的其他组件/工具 → 不在此组件测试，由该组件自己的测试覆盖（如 `math::remap` 属 QoolCommon 数学工具，不在 mapper 组件重复测）
...
- 组件依赖的 Qt 引擎行为不是组件契约：QQmlListProperty 的 list 索引/元素访问、SignalSpy 时序、QML 默认属性 append 路径等——不替 Qt 验证
- 组件没承诺的能力不测：list 属性无 at 函数（元素访问 null）是设计如此不是缺陷——不修实现、不写对应断言（「测试对象缺失能力」是无效断言）
- 区分「设计如此」与「契约缺陷」以文档为准：文档定义了而实现不符 = 缺陷修实现；文档没定义的 = 不是缺陷，不修不测
- 与「契约裁剪项显式验证『无』」的区分：裁剪项是文档明确声明无的能力（无 `reset`/无 `defaultValue`）——显式断言其不存在以锁定裁剪；未承诺能力是文档从未提及的引擎行为——不测。前者测「文档说没有」的，后者不测「文档没说有」的

### 不为测试提供专用功能

- 组件缺失的能力靠 debug 信息/读源码探测，不因测试需要而补实现（如给 list 属性补 `at` 函数只为测试索引访问——那是为测试造功能）
- 测试需要的行为面如果组件本就不提供，说明该行为不是契约——删断言而非改组件

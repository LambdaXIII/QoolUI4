...
- **qt.conf**（Windows 前缀解析）：`qt.conf.in` 配置期生成到 core/qml 层 exe 旁（`Prefix = Qt 安装前缀`，绝对路径正斜杠）——QLibraryInfo 读取，覆盖 DLL 复制导致的推导失效；任何运行通道无需环境注入；源码不含安装路径。**common 层不需要**（纯 QCoreApplication，不加载平台插件）；core 层需要（链接 Qt6::Quick → QGuiApplication 构造加载平台插件）。改模板后需重新 configure 生效

...
// （Components 基底）+ 滚动（官方 ScrollView 组合）。
//
// 滚动：官方「Scrollable TextArea」配方——QC.ScrollView（Controls 版，
// 非 T.ScrollView——T 版无 TextArea 内容尺寸接驳，实测裁定 2026-08-11）
// 对 TextArea 的集成（内容尺寸自动/背景不滚/clip 自动）；滚动条挂 Qool
// 主题件（垂直/水平均 Qool ScrollBar——非 Qt 默认样式；水平 AsNeeded，
// 默认 Wrap 下不出现）。
//
...
/*!
    ...
    继承 Qt Quick Controls 的 \l ScrollView（其继承 Pane）——官方 API 全部
    可用（contentData / effectiveScrollBarWidth / effectiveScrollBarHeight
    等），宿主可参照官方文档。本类型不改变官方行为，仅预设 Qool 主题滚动条
    与文本 API 转发，以下仅文档化 Qool 设置与转发部分。
    ...
    垂直滚动条为 Qool ScrollBar（主题样式、可交互拖动），水平滚动条同为
    Qool ScrollBar（AsNeeded——默认 Wrap 折行下不出现）。内容尺寸随文本
    自动增长、背景装饰不随内容滚动、自动裁剪——ScrollView 对 TextArea 的
    官方集成保证。

...
// BasicBeeperApp：Beeper 应用基类——安装在 Beeper 上、订阅其消息的
// 应用外壳。经 Beeper 的 apps 默认属性安装（target 自动指向宿主
// Beeper）后，Beeper 收到的消息被转发为 messageRecieved 信号。子类
// （如 MessageLogger）重写 appName 并提供业务逻辑。
//
// target 安装/卸载（刻意设计）：target 指向所安装的 Beeper。换绑时
// targetChange 只断开 oldTarget → 本对象的连接——刻意不用单参数
// disconnect(oldTarget)（那会断开 oldTarget 的全部出站连接，破坏
// Beeper 的消息投递链）。卸载日志刻意引用 oldTarget（newTarget 可能
// 为 null，解引用即崩溃）。
//
// appID：由 appName + '_' + 构造时生成的随机串组成，每个实例唯一；
// appName 由子类重写（基类自身返回 "BasicBeeperApp"）。
//
// QML_ANONYMOUS：不可在 QML 中直接实例化，仅作为派生类型的基类存在。
